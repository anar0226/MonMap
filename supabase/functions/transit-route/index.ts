...
async function fetchAllStops(): Promise<BusStop[]> {
  ...
  // raw is array of { busstop_id, busstop_nmmn, busstop_nmus, gps_coordy, gps_coordx }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const legs = await findTransitLegs(from_lng, from_lat, to_lng, to_lat);
    return new Response(
      JSON.stringify({ legs, fetchedAt: new Date().toISOString() }),
